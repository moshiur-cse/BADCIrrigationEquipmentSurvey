    ...
    if (all_data && all_data[adminCode]) {
        try {
            map_data = all_data[adminCode].map(function(d, i) {
                return { geo_code: d[adminCode + "Code"], data_value: parseInt(d[dataCode]) };
            });
...
function map_init(adminCode) {
    ...
    for (var dc = 0; dc < map_data.length; dc++) {
        ...
        //console.log(currVal + " : " + minVal + " : " + maxVal + " : " + totalValue);
    ...
    //console.log(maxVal + " : " + minVal + " : " + clrClass.length + " : " + delt);
